...
class App extends Component {
    ...
    spotifyWebAPI
      ...
      .then(response => {
        ...
        if (this.state.selectedPlaylistSongs.length < response.total)
          this.getSavedSongs(limit, offset + limit);
      });
  }
    ...
    Promise.all(
      ...
      for (arr of response) {
        for (song of arr.audio_features) {
          ...
          if (song.energy > 0.8 || song.danceability > 0.8) mood = "hype";
          else if (
            song.energy > 0.65 &&
            ...
            mood = "good vibes";
          else if (song.valence < 0.5 && song.energy < 0.5) mood = "bag";
          else mood = "chill";
      ...
      moodNames.map(mood => {
        spotifyWebAPI
          .createPlaylist(this.state.userID, {
            name: mood + " songs of " + this.state.chosenPlaylist.name,
            description:
              "Created by mood-sort from " +
          ...
          .then(response => {
            if (mood === "sad" && bagURIs[0].length !== 0) {
              ...
              goodVibesURIs.map(goodVibesURI => {
                spotifyWebAPI.addTracksToPlaylist(
                  "",
                  response.id,
                  goodVibesURI
                );
  ...
  handleMoodChange(e) {
    ...
    this.setState(prevState => ({
      moods: prevState.moods.set(item, isChecked)
    }));
  }
